...
public class IntelPerC : MonoBehaviour {
	...
	Vector2 faceRectCenter=Vector2.zero; int lastFaceLeftNeutralRight=0;
	...
	public bool GetFaceLookLeft(){
		if(confidence>80&&faceRectCenter.x>390&&lastFaceLeftNeutralRight!=-1){
			lastFaceLeftNeutralRight=-1;
			return true;
		}else{lastFaceLeftNeutralRight=0; return false;}
	}
	public bool GetFaceLookRight(){
		if(confidence>80&&faceRectCenter.x<30&&lastFaceLeftNeutralRight!=1){
			lastFaceLeftNeutralRight=1;
			return true;
		}else{lastFaceLeftNeutralRight=0; return false;}
	}	
	...
	void Update () {
		...
		if(pp.QueryFaceID(0,out face,out timestamp)){
			...
			if(pp.QueryFaceLocationData(face,out datafacedetect)){
				if(datafacedetect.confidence>20){
		...
		if(pp.QueryGeoNode(PXCMGesture.GeoNode.Label.LABEL_BODY_HAND_PRIMARY,data)){
			...
			// z in
			if(confidence>80)
				worldPosition = new Vector3(-data[0].positionWorld.x*2f,data[0].positionWorld.z*3f+0.15f,-data[0].positionWorld.y+0.3f)*15f;
			worldPositionThumb = new Vector3(-data[1].positionWorld.x,data[1].positionWorld.z,-data[1].positionWorld.y);
			worldPositionIndex = new Vector3(-data[2].positionWorld.x,data[2].positionWorld.z,-data[2].positionWorld.y);
			worldPositionMiddle = new Vector3(-data[3].positionWorld.x,data[3].positionWorld.z,-data[3].positionWorld.y);
			worldPositionRing = new Vector3(-data[4].positionWorld.x,data[4].positionWorld.z,-data[4].positionWorld.y);
			worldPositionPinky = new Vector3(-data[5].positionWorld.x,data[5].positionWorld.z,-data[5].positionWorld.y);
